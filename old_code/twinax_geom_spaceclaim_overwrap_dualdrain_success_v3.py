# Python Script, API Version = SpaceClaim V22
# Robust Cable Geometry Generation with or without Drains, shell or fill options, elliptic or multilumen
...
is_a_doublet = int(get_param('is_a_doublet', 0)) # 0 = Separate, 1 = Combined
...
# Mapping W_outer and H_outer to the image variables D and H
# W_outer is the full width, so D = W_outer / 2
# H_outer is the full height, so H = H_outer / 2
...
def sketch_elliptic(W_val, H_val, h):
    ...
    # Calculate rs and rx based on your image formulas
    ...
    # 1. Create Central Ellipse using the DirectionUV discovered in your recording
    # We use rx as the major radius and H as the minor radius
    SketchEllipse.Create(origin, DirectionUV.DirU, DirectionUV.DirV, MM(rx), MM(H))
    
    # 2. Side Circular Arcs (Side Caps)
    side_center_x = D - rs
    sketch_circle(side_center_x, 0, rs)
    sketch_circle(-side_center_x, 0, rs)
    
def sketch_profile(dx, R, W_val, H_val):
    """Switcher between Double-D and Elliptic"""
    if is_elliptic:
...
if is_a_doublet and filler_opt == 1:
    ...
    if R_fill_inner > 0:
        sketch_profile(dx_in, R_fill_inner, W_in, H_in)
