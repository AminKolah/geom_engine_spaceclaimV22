...
if filler_mode.lower() == "fill":
    ...
    # Calculate shell thickness based on gap between core and shield side
    t_filler_shell = (dx_in + R_in) - (dx_cond + r_core)
    
    # Outer boundary (Shield Inner)
    sketch_doubleD(dx_in, R_in)
    
    # Inner boundary (Offset inward)
    R_fill_inner = R_in - t_filler_shell
    dx_fill_inner = dx_in # Keeping the flat section length
    
    # Only sketch if geometry is valid
    if R_fill_inner > 0:
        sketch_doubleD(dx_fill_inner, R_fill_inner)
    
    # Still add core holes if they intersect the shell band
    sketch_circle(-dx_cond, 0, r_core)
    sketch_circle(dx_cond, 0, r_core)
    
    # Extrude the resulting band
    extrude_and_name("Second_Extrusion", L_extrude, True)
...
if drain_opt:
    ...
    # Standard Double-D overwrap (Hugging the Shield)
